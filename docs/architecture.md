...
| `/reports` | Приватний API ядра: draft, stone, lifecycle, події, RBAC і server-paginated dashboard list (`items`, `total`, сторінки, пошук і allow-list фільтрів/сортувань). Для локального demo-набору список також повертає legacy `price`, який UI маркує `USD … d`; це не ринкова чи експертна ціна. |
