...
> **Статус на 17 вересня 2026.** Працює локальний контур: frontend на Pug/SCSS/JavaScript збирається Gulp і віддається BrowserSync; FastAPI надає JSON API та JWT-вхід; SQLAlchemy працює з MariaDB у XAMPP. Revisions `0002_report_core`–`0009_nbu_fx_snapshots` формують ядро, private files, authoring-вимоги, revocable public passport, immutable metadata ruleset-ів, versioned market snapshots і frozen NBU FX snapshots. `0009` застосовано до локальної MariaDB 17 вересня 2026. Dashboard, wizard і private detail/edit використовують лише `/reports`; legacy `/diamonds/*` вилучено без міграції historical колонок. Docker, PostgreSQL і завершений ML-потік ще не реалізовані.
...
| Група | Призначення |
| --- | --- |
| `/reports` | Приватний API ядра: draft, stone, lifecycle, події, RBAC, server-paginated dashboard list і full detail/update. `PUT /reports/{id}` допускається тільки для draft owner/admin та створює append-only `report_updated`; transitions лишаються окремим endpoint-ом. Dashboard передає `sold=true|false`; це зручна двостанова проєкція фактичного `market_status` (`sold` / усі інші стани), а не втрата його деталізації. Для локального demo-набору список також повертає legacy `price`, який UI маркує `USD … d`; це не ринкова чи експертна ціна. Підтримуваний draft best-effort отримує `USD … *` (`system_market_reference`) з останнього approved OpenFacet snapshot-а і frozen НБУ; вручну підтверджений admin-ом `USD … of` має пріоритет. |
...
| `/market-data/*`, `/reports/{report_id}/valuations*` | Admin-only provider-neutral контур: OpenFacet candidate → approve/reject, NBU USD/UAH refresh і ручне підтвердження `market_reference` до конкретного звіту. `POST/PUT /reports` best-effort створює `system_market_reference` за останнім approved OpenFacet snapshot-ом, якщо stone покритий; немає fallback до старого FX і помилка enrichment не блокує draft. Усі valuation містять immutable USD, UAH, rate та official date. Public passport і PDF сюди не підключені. |
