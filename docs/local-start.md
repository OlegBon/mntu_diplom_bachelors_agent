...
Для наявної локальної БД спочатку виконай лише read-only перевірку:
...
Якщо схема відповідає стартовій ревізії, її можна позначити командою
`stamp 0001_initial_schema`. Це записує версію в `alembic_version`, тому
виконуй stamp лише після резервної копії та окремого підтвердження. Не
застосовуй `downgrade` до БД із потрібними даними: початковий downgrade видаляє
таблиці.
