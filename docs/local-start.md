...
довідники geometry, не змінюючи legacy-записи. Revision `0005_public_passports`
...
Gulp/Browsersync зазвичай віддає збірку на `http://localhost:3000`; якщо порт зайнятий, він обере інший локальний порт. API дозволяє CORS-запити лише з `localhost` або `127.0.0.1` з номером порту, тому зміна локального порту не блокує вхід. `frontend/dist` генерується автоматично. BrowserSync не дзеркалить кліки й введення між відкритими локальними вікнами (`ghostMode: false`), тому збереження або завантаження PDF виконується лише у вікні, де натиснуто дію.
