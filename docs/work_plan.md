...
- [x] 095 — Передача публічного паспорта та PDF: admin бачить і копіює код/URL, landing приймає code або QR URL, server генерує on-demand allow-listed PDF для поточного active issued report. Старий QR/URL не підходить після reissue; revoke і void закривають нове завантаження.
