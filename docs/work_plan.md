...
- [x] 121 — Розширення ринкових провайдерів і FX: НБУ USD/UAH fetch/ручний refresh, immutable FX snapshot і policy майбутніх системних орієнтирів реалізовано у `0009`/`0010`; у 162 policy стала enabled provider set із nullable dashboard primary. Admin вмикає provider-и checkbox-ами та вмикає/вимикає UAH-конвертацію НБУ; майстер preview читає policy й показує незалежні нефіксовані USD-орієнтири. Dashboard/private detail показують provenance. Операційний контур завершено у 122; його актуальний опис — [market-data-providers.md](./guides/market-data-providers.md).
...
- [ ] [167 — Synthetic demo dataset date-boundary renewal](./backlog/167-synthetic-demo-dataset-date-boundary-renewal.md): v2 generator і read-only inventory готові; після backup/restore verification виконати явну replacement-команду `--replace-v1 --confirm-replace-v1` з тими самими `DEMO-…` ID та останньою датою 31.12.2025; не зачіпає historical `DR-*`.
