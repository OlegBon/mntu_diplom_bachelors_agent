...
- [x] [121 — Розширення ринкових провайдерів і FX](./backlog/121-market-data-provider-expansion-and-fx.md): НБУ USD/UAH fetch/ручний refresh та immutable FX snapshot реалізовано у `0009`; підтримуваний новий або змінений draft автоматично отримує `system_market_reference` з останнього approved OpenFacet snapshot-а, а ручний `market_reference` admin-а лишається пріоритетним. Dashboard/private detail показують frozen UAH. Scheduler, нові комерційні провайдери і будь-яке public/PDF-відображення лишаються окремими рішеннями.
