...
- [ ] **mariadb-local-recovery:** перенести локальні бази Diamant ID у чисту
  MariaDB/XAMPP інсталяцію з перевірених SQL-дампів. Поточний XAMPP після
  інциденту є тимчасовим джерелом експорту; не застосовувати до нього нові
  migrations або seed до завершення переходу. Runbook:
