...
- **Рішення:** `GET /reports` повертає `{items,total,page,page_size,total_pages}`, обмежує `page_size` до 100, застосовує allow-list сортувань, пошук за `report_id`, lifecycle і market-status фільтри. Gemologist бачить лише власні записи; admin бачить усі або одного обраного експерта. Dashboard повернуто до table-first вітрини з 4C і бейджами: пошук і сортування застосовуються одразу, розгорнуті фільтри — через «Застосувати». Значення legacy `price` demo-набору 2023–2025 показується як `USD … d`; натискання пояснює джерело, дату і межі набору. Це не поточне котирування, не ціна продажу і не механізм переоцінки історії. Колонка «Дії» має доступний overflow-control `⋮`; пункти detail/edit/print вимкнені до задачі 080, а bulk-операції не додані.
