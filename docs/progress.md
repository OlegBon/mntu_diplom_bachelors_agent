...
- **Змінені файли:** `docs/{backlog/{README,133-local-mvp-release-audit (видалено),134-alembic-schema-convergence,135-real-browser-e2e-and-passport-contract}.md,decisions/{001-report-domain-contract,003-report-core-migration-plan}.md,guides/{idc-demo-v1-ruleset,product-ux-foundation}.md,reviews/2026-09-24-local-mvp-release-audit.md,work_plan,progress}.md`, `scripts/check_doc_links.py`, `tests/unit/test_doc_links.py`.
- **Рішення / Результат:** критичних functional/security знахідок у перевіреному local зрізі немає. Виявлено дві release-важливі прогалини: `alembic check` бачить index/metadata drift при MariaDB revision `0013`, а Playwright passport delivery mock не відповідає чинному wrapper-контракту й дає 16/17. Їх не виправляли неявно: створено 134 для schema convergence і 135 для deterministic passport contract та isolated real browser E2E. Completed backlog links замінено durable docs/plain history; додана dependency-free перевірка локальних Markdown-посилань.
