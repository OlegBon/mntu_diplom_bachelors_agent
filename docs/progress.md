...
- **Перевірки:** `python -m pytest tests/unit/test_migration_foundation.py tests/unit/test_market_provider_operations.py tests/api/test_market_data.py -q` — 14 passed; `python -m compileall -q backend scripts`; FastAPI import smoke; `alembic upgrade head --sql` містить обидві нові таблиці без локального write; `frontend npm test` — 20 passed; Playwright `market-data.spec.mjs` — 1 passed; `git diff --check` — без помилок. In-app browser runtime не запустився через відсутній системний шлях, тому rendered QA виконано штатним Playwright тестом проєкту.
- **Нові змінні середовища:** немає.
- **Обмеження:** `0013` ще не застосована до локальної MariaDB; не додається комерційний provider, cloud scheduler, alerting-канал чи автоматичне approve OpenFacet candidate. Public passport і PDF не містять ринкових сум.
