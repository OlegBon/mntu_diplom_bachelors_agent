...
- **Рішення / Результат:** 156 позначена як застосована `0014` без historical classification backfill; активна послідовність починається з `162 → 157 → 158 → 159`, без повторного посилання на вже виконану задачу.
- **Перевірки:** звірено active backlog і фактичну Alembic head `0014_demo_dataset_isolation` локальної MariaDB.
- **Нові змінні середовища:** немає.
- **Обмеження:** це документаційне уточнення; generator, demo preview/PDF, SOM і historical classification не реалізовані.

## 2026-09-25 — demo-dataset-isolation

- **Задача:** виконати 156 — ізолювати synthetic demo dataset від operational workflow, публічної видачі та operational-аналітики.
- **Змінені файли:** `backend/{models,schemas,crud,main}.py`, `alembic/versions/0014_demo_dataset_isolation.py`, `frontend/src/{pug/pages/dashboard.pug,js/modules/dashboard.js}`, `scripts/inventory_demo_seed.py`, `tests/{api/test_demo_dataset_isolation.py,unit/test_demo_seed_inventory.py,unit/test_schema_metadata.py,unit/test_migration_foundation.py}`, `docs/{architecture,db-schema,progress,work_plan}.md`, `docs/{decisions/006-demo-dataset-isolation.md,guides/{README,current-domain-and-report-workflow,demo-dataset-operations,market-data-providers}.md}`; завершений backlog-файл `156-demo-dataset-contract-and-isolation.md` вилучено.
- **Рішення / Результат:** `DiamondReport` отримав server-enforced `record_scope` та nullable manifest FK `demo_dataset_id`; immutable `demo_datasets` містить version/provenance/checksum/allow-list аналітичних сценаріїв. Звичайні `/reports`, dashboard, counters, review stats, passport/QR/PDF/public media та next `DR-…` працюють лише з operational data. Admin може лише читати конкретний dataset через explicit `/demo/datasets/...`; gemologist отримує opaque `404`, а всі regular demo writes відхиляються. Dashboard більше не показує legacy `d`: reference type і source розділені як `SYS · provider` або `ADM · provider`. System-origin synthetic event/valuation/media — `NULL`, а не реальний користувач.
- **Перевірки:** цільові API/unit regression тести із demo scope, manifest allow-list, opaque access, public isolation, next ID, metadata й schema-only migration; `npm test` (25) і `npm run test:e2e` (22) green. Застосовано `0014`, `alembic current` підтверджує head, `alembic check` — без нових upgrade operations. Read-only `inventory_demo_seed.py` підтвердив 1 000 historical ID без missing/unexpected ID, без report без Stone та orphan events; `DR-01001+` — 9 operational records.
- **Нові змінні середовища:** немає.
- **Обмеження:** застосована `0014_demo_dataset_isolation` не створює manifest/demo records і не backfill-ить `DR-00001…DR-01000`; для цього потрібні backup і окреме явне погодження classification backfill. Generator, private demo preview/PDF і SOM ще належать задачам 157–159.
...
- **Змінені файли:** `frontend/src/{pug/layout/main.pug,pug/pages/{privacy,documentation,dashboard}.pug,scss/_ui-primitives.scss}`, `frontend/tests/{page-dom.test.mjs,e2e/public-information-pages.spec.mjs}`, `docs/{work_plan,progress}.md`, `docs/backlog/{README.md,132-public-information-pages.md (видалено)}`.
- **Рішення / Результат:** footer тепер веде на «Політику конфіденційності» та «Документацію». Privacy-сторінка описує фактичний local MVP: account/profile/report/media data, role-based доступ, public passport, локальне runtime-сховище та JWT у браузері; вона явно не видає себе за фінальну legal/production політику. Перед privacy-notice додано короткий змістовний вступ, тож виділений блок має однаковий і природний ритм із секцією. Public documentation пояснює code/direct URL/QR, межі публічних даних, статус актуальності, PDF і медіа. Обидві сторінки мають стандартний широкий контейнер, vertical rhythm, заголовок і responsive semantic layout, як інші робочі екрани; notice має однакові зовнішні відступи незалежно від місця в тексті. Підзаголовки «Всі звіти» та «Новий звіт» отримали завершальну крапку, як завершені речення на інших сторінках.
- **Перевірки:** `cmd /c "cd frontend && npm test"` — 22 passed; Playwright `public-information-pages.spec.mjs` — desktop і mobile успішні; `git diff --check` — без помилок.
- **Нові змінні середовища:** немає.
...
- **Змінені файли:** `frontend/src/{pug/pages/index.pug,js/main.js}`, `frontend/tests/page-dom.test.mjs`, `docs/progress.md`.
- **Рішення / Результат:** anonymous visitor як і раніше бачить «Увійти для роботи зі звітами». За наявності локальної сесії frontend приховує цей CTA під час ініціалізації сторінки; заголовок і навігація лишаються єдиним місцем переходу до робочого середовища.
- **Перевірки:** `cmd /c "cd frontend && npm test"` — 21 passed; `git diff --check` — без помилок.
- **Нові змінні середовища:** немає.
...
- **Змінені файли:** `backend/{crud,main,media_storage,schemas}.py`, `frontend/src/{pug/pages/passport,report-detail}.pug,scss/_ui-primitives.scss,js/modules/{api,public-passport,report-detail}.js}`, `tests/api/{test_public_passport,test_public_passport_media}.py`, `frontend/tests/e2e/{public-passport,report-detail}.spec.mjs`, `docs/{architecture,db-schema,work_plan,progress}.md`, `docs/guides/current-domain-and-report-workflow.md`, `docs/backlog/{README.md,130-public-passport-media.md (видалено)}`.
- **Рішення / Результат:** усі вкладення стартують private. Лише admin і лише для `issued` report може окремо увімкнути public-доступ для `stone_photo` або `plotting_diagram`; у private UI вони названі «Фото каменю» та «Схема огранювання (plotting)». Кнопка має окремий відступ і на час запиту показує «Публікація…» та блокує повторний клік. Публікацію та відкликання media підтверджує стандартний діалог проєкту, не browser alert; на mobile він має природну висоту за вмістом, а не розтягується до низу екрана. Він пояснює наслідок для чинного посилання/QR і не впливає на PDF. Private `GET /reports/{id}/passport` повертає `200` з `passport: null`, якщо паспорт ще не опубліковано, тому цей нормальний UI-стан не створює 404 у browser console. Anonymous API перевіряє active issued passport token, exact asset allow-list, JPEG/PNG/WebP MIME, наявність файлу й SHA-256; у public response немає filename, hash або private metadata. Content віддається з `Cache-Control: no-store` та `X-Content-Type-Options: nosniff`; revoke, reissue старого token-а, void, private або tampered asset повертають 404. PDF лишається без вкладень.
- **Перевірки:** targeted API tests — 6 passed; 31 unit/domain/PDF tests; після UI/contract polish: `pytest tests/api/test_public_passport.py tests/api/test_public_passport_media.py -q` — 4 passed, `npm test` — 20 passed, targeted Playwright `report-detail.spec.mjs` — 2 passed (включно з публікацією паспорта, стилями діалогу та mobile-висотою); `git diff --check` перевірено.
- **Нові змінні середовища:** немає.
...
- **Змінені файли:** `alembic/versions/0013_market_provider_operations.py`, `backend/{models,schemas,crud,main,market_operations}.py`, `scripts/run_market_provider_schedule.py`, `scripts/audit-api.mjs`, `tests/unit/{test_migration_foundation,test_market_provider_operations}.py`, `frontend/src/{pug/pages/market-data.pug,js/modules/{api,market-data}.js}`, `frontend/tests/{auth-and-api,e2e/market-data}.mjs`, `docs/{architecture,db-schema,local-start,api-mvp-audit,work_plan,progress}.md`, `docs/guides/market-data-providers.md`, `docs/backlog/{README,122-market-provider-operations (видалено)}.md`.
- **Рішення / Результат:** `0013` додає `market_provider_schedules` і append-only `market_provider_operations`; початкові графіки — OpenFacet 08:30 та НБУ 15:40 за `Europe/Kyiv`, з warning/block порогами та retries 15/30/60 хв. Зовнішній Windows/cloud scheduler викликає окремий CLI, а FastAPI не створює фоновий daemon. Scheduled OpenFacet створює лише candidate, NBU дедуплікує однакові rate/date. Save/update report і ручний attach не виконують зовнішній fetch: використовують лише cached НБУ snapshot у block-порозі. Стара history, valuation, demo `d`, passport/PDF не змінюються. Admin UI показує freshness, налаштування графіків і останні операції. Після реального запуску виправлено вибір останнього snapshot-а: замість `scalar()` використовується впорядкований `first()`, тож кілька FX-знімків не дають 500.
- **Перевірки:** `python -m pytest tests/unit/test_migration_foundation.py tests/unit/test_market_provider_operations.py tests/api/test_market_data.py -q` — 14 passed; після fix `python -m pytest tests/unit/test_market_provider_operations.py tests/api/test_market_data.py -q` — 8 passed; `python -m compileall -q backend scripts`; FastAPI import smoke; `alembic upgrade head --sql` містить обидві нові таблиці; фактичний `alembic upgrade 0013_market_provider_operations` застосовано до локальної MariaDB, `alembic current` — `0013_market_provider_operations (head)`, read-only SQL підтвердив обидві таблиці та стартові графіки. `frontend npm test` — 20 passed; Playwright `market-data.spec.mjs` — 1 passed; `git diff --check` — без помилок. In-app browser runtime не запустився через відсутній системний шлях, тому rendered QA виконано штатним Playwright тестом проєкту.
- **Нові змінні середовища:** немає.
- **Обмеження:** не додається комерційний provider, cloud scheduler, alerting-канал чи автоматичне approve OpenFacet candidate. Public passport і PDF не містять ринкових сум.
...
- **Рішення / Результат:** `0012` додає nullable `first_save_started_at`/`time_to_first_save_seconds` до нового report та `wizard_work_sessions` як короткоживучий технічний lease; міграцію застосовано до локальної MariaDB. Лише видима дія gemologist стартує серверний lease; `POST /reports` атомарно claim-ить власний непрострочений UUID, записує elapsed duration і видаляє lease. Інша вкладка замінює попередній lease, а stale/offline/покинута спроба не блокує save й не створює operational metric. У деталях експерта metric показується окремими count/total/average/median, не змішаними з active-time saved draft. Покроковий workflow тепер прямо розрізняє обидва виміри.
- **Перевірки:** `python -m pytest tests/api/test_report_domain.py tests/unit/test_migration_foundation.py tests/api/test_wizard_first_save_time.py tests/api/test_expert_statistics.py -q` — 20 passed; `python -c "from backend.main import app; print(app.title)"` — успішно; `cmd /c "cd frontend && npm test"` — 20 passed; Playwright wizard + analytics — 2 passed; SQL-render і застосування `0012_wizard_first_save_time`, після чого перевірено revision, таблицю й колонки; ручна перевірка створення report і active-time сесії — успішна; `git diff --check` — без помилок.
- **Нові змінні середовища:** немає.
- **Обмеження:** це elapsed time, а не active-time; максимальний строк lease — 4 години. Немає historical backfill або scheduler-а для фонової очистки покинутих lease: прострочені записи прибираються при наступному start майстра.
