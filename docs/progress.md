...
- **Рішення:** `GET /reports` повертає `{items,total,page,page_size,total_pages}`, обмежує `page_size` до 100, застосовує allow-list сортувань, пошук за `report_id`, швидкі lifecycle/market-status фільтри, а в розгорнутій панелі — форму, 4C, діапазони ваги/ціни та дати. Gemologist бачить лише власні записи; admin бачить усі або одного обраного експерта. Dashboard повернуто до table-first вітрини з 4C і двома відокремленими статусами: «Статус звіту» та «Статус продажу». Заголовки колонок запускають server-side сортування, дата знов показує час. Значення legacy `price` demo-набору 2023–2025 показується як `USD … d`; натискання пояснює джерело, дату і межі набору, клік поза popover або `Esc` закриває його. Це не поточне котирування, не ціна продажу і не механізм переоцінки історії. Колонка «Дії» має доступний overflow-control `⋮`, який також закривається кліком поза меню або `Esc`; пункти detail/edit/print вимкнені до задачі 080, а bulk-операції не додані.
