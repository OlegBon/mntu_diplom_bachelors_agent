...
- **Перевірки:** цільові API/unit regression тести із demo scope, manifest allow-list, opaque access, public isolation, next ID, metadata й schema-only migration; frontend build/test запускаються перед фінальним handoff. `scripts/inventory_demo_seed.py` перевіряє historical range тільки читанням.
- **Нові змінні середовища:** немає.
- **Обмеження:** `0014_demo_dataset_isolation` підготовлена, але не застосована до локальної MariaDB. Вона не створює manifest/demo records і не backfill-ить `DR-00001…DR-01000`; для цього потрібні backup, read-only inventory та окреме явне погодження. Generator, private demo preview/PDF і SOM ще належать задачам 157–159.
