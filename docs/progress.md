...
- **Перевірки:** `python -m pytest` — 23 passed; `npm test` — 13 passed; targeted Playwright public passport — 1 passed; full mock E2E запущено для наявних flows. `git diff --check` — успішно. API regression перевіряє RBAC, allow-list, QR SVG, reissue, revoke і void. Після застосування міграції `npm run audit:api` — 10/10. Додатковий Playwright regression підтвердив: для report `review` admin бачить лише пояснення, без QR і дій публікації.
