...
- `scripts/seed_db.py` — актуальний руйнівний seed. `scripts/seed_db-start.py` — старий альтернативний сценарій: використовує застаріле поле `evaluation_time_min` і не повинен запускатися; його потрібно окремо вилучити або заархівувати після підтвердження.
