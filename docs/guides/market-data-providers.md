...
У «Всі звіти» заголовок `Ціна (USD)` пояснює, що колонка містить довідковий
орієнтир, а не ціну продажу. Біля значення показується незалежний тип і джерело:
`SYS · OpenFacet` для системного орієнтиру та `ADM · OpenFacet` для орієнтиру,
застосовність якого окремо підтвердив адміністратор.
