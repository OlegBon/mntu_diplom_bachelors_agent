...
Alembic revision `0012_wizard_first_save_time`, готову до застосування після
локальної `0011_expert_work_sessions`. `0011` уже застосована до локальної MariaDB;
`0012` не містить backfill і очікує окремої команди. Це карта даних для розробки, API та
